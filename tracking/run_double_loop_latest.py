...
parser.add_option(
    "--rt_underbid_penalty",
    dest="rt_underbid_penalty",
    help="Set the penalty for underbid at real-time.",
    action="store",
    type="float",
    default=600.0,
)
...
parser.add_option(
    "--sim_batch",
    dest="sim_batch",
    help="Set simulation batch number.",
    action="store",
    type="str",
    default='3r',
)
...
rt_underbid_penalty = options.rt_underbid_penalty
sim_batch = options.sim_batch
...
rts_gmlc_data_dir = (
    "/afs/crc.nd.edu/user/x/xchen24/GitHub/RTS-GMLC/RTS_Data/SourceData/"
)
output_dir = f"batch_{sim_batch}_sim_{sim_id}_results"
...
if participation_mode == "Bid":
    bidder_object = Bidder(
        ...
        real_time_underbid_penalty=rt_underbid_penalty,
    )
elif participation_mode == "SelfSchedule":
